...
function isRealError (error) {
  if (
    /Cannot associate course ".+?" - is associated to another blueprint course/.test(
      error
    )
  ) {
    return false
...
async function getSisErrors (sisImportId) {
  const errors = []
  const errorPages = canvasApi.listPaginated(
    `accounts/1/sis_imports/${sisImportId}/errors`
  )
...
async function sendDirectory (dir) {
  ...
  log.info(`SIS IMPORT: correctly created with ID [${response.id}]`)
