      ...
      `${process.env.AKTIVITETSTILLFALLEN_API_URL}/aktivitetstillfallen/students?fromDate=${date}&toDate=${date}`,
      {
        ...
        retry: {
          ...
          calculateDelay: ({attemptCount, retryOptions, error, computedValue}) => {
            // This function replaces the default function. The reason is mainly to 
            // log each time a retry is performed
            log.info(`An error occurred when trying to reach the akt api: ${error}. Retrying with attempt:, ${attemptCount}/${NUMBER_OF_RETRIES}`)
...
async function writeExaminers (activities, file) {
  ...
  normalizedExaminers.forEach(a => {
    stream.write({
      section_id: `AKT.${a.activity_ladok_uid}.${a.date}`,
    ...
    stream.write({
      section_id: `AKT.${a.activity_ladok_uid}.${a.date}.FUNKA`,
      user_id: a.examiner_user_id,
      role_id: EXAMINER_ROLE_ID,
      status: 'active'
    })
...
async function getActivitiesByDate (start, end) {
  let result = []
  for (
    ...
    result.push(...(await getActivities(dateString)))
  }
  return result
}
...
async function writeActivities (start, end, dir) {
  let activities = await getActivitiesByDate(start, end)
