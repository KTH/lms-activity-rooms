...
const NUM_OF_DAYS_TO_USE_FOR_ENDDATE = 60
const NUM_OF_DAYS_FORWARD_TO_HANDLE = 10
...
/**
 * Only set end dates in the near future, to bypass the Canvas feature where enddates are
 * cleared by blueprint courses upon creation, but not on updates
 * */
function endDateIfNear (activity) {
  const controlDate = new Date()
  controlDate.setDate(controlDate.getDate() + NUM_OF_DAYS_FORWARD_TO_HANDLE)

  const examDate = new Date(activity.date)
  if (examDate.getTime() >= controlDate.getTime()) {
    // The examDate is too far in the future, don't use endDate
    return null
  } else {
    // The examDate is in the near future, use endDate
    const endDate = new Date(activity.date)
    endDate.setDate(endDate.getDate() + NUM_OF_DAYS_TO_USE_FOR_ENDDATE)
    return endDate
  }
}
...
async function getActivities (date) {
  try {
    ...
    const result = body.aktivitetstillfallen.map((akt, i) => {
      const raw = body.aktivitetstillfallenraw[i]
      const registrationPeriod = raw.Anmalningsperiod
      const name = raw.Benamning.sv
...
async function writeCoursesAndSections (activities, coursesFile, sectionsFile) {
  ...
  for (const activity of activities) {
    ...
    normalized.push({
      ...
      school: schools[0],
      name: `${activity.name} för ${codes}: ${activity.date}`,
      date: activity.date
    })
  }
  ...
  normalized.forEach(a => {
    ...
    stream1.write({
      ...
      status: 'active',
      blueprint_course_id: process.env.BLUEPRINT_SIS_ID,
...
async function getStudents (activities) {
  ...
  for (const activity of activities) {
    if (!activity.registrationPeriod) {
