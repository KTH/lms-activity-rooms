...
async function writeExaminers (activities, file) {
  ...
  for (const activity of activities) {
    ...
    log.info(`Activity ${activity.ladokUID}. Added ${examiners.length} examiners`)
...
async function writeStudents (activities, file) {
  ...
  for (const activity of activities) {
    if (new Date(`${activity.registrationPeriod.Slutdatum}T23:59:59Z`) >= new Date()) {
      log.info(`Activity ${activity.ladokUID}. Registration not closed. It closes on ${activity.registrationPeriod.Slutdatum}`)
      continue
    }
