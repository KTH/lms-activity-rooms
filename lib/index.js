...
const { sendDirectory, getSisErrors } = require('./canvas')
  ...
  try {
    ...
    const result = body.aktivitetstillfallen
      .map((akt, i) => {
        ...
        const name = raw.aktivitetstillfalle.Benamning.sv
    ...
    log.info(
      `Date ${date}. Obtained ${result.length} activities`
    )
...
async function writeCoursesAndSections (activities, coursesFile, sectionsFile) {
  ...
  normalized.forEach(a => {
    ...
    stream2.write({
      course_id: `AKT.${a.ladok_uid}`,
      section_id: `AKT.${a.ladok_uid}.${a.date}`,
    ...
    stream2.write({
      course_id: `AKT.${a.ladok_uid}`,
      section_id: `AKT.${a.ladok_uid}.${a.date}.FUNKA`,
...
async function writeExaminers (activities, file) {
  ...
  for (const activity of activities) {
    ...
    for (const examiner of examiners) {
      normalized.push({
        // TODO: rename this to activity_ladok_uid
        ladok_uid: activity.ladokUID,
        // TODO: rename this to examiner_user_id
...
async function writeStudents (activities, file) {
  // TODO: rename to normalizedActivities
  const normalized = []
  ...
  for (const activity of activities) {
    ...
    for (const student of students) {
      normalized.push({
        // TODO: rename to student_user_id
        user_id: student.kthid,
        section_id:
          student.funka.length > 0
            ? `AKT.${activity.ladokUID}.${activity.date}.FUNKA`
            : `AKT.${activity.ladokUID}.${activity.date}`
      })
    }
  }
...
async function writeActivities (start, end, dir) {
  ...
  for (
    ...
    // TODO: why not use activities.push(...(await getActivities(dateString)) instead which is clearer IMO
...
async function syncActivities (start, end) {
  ...
  const sisImport = await sendDirectory(dir)
  const url = `${process.env.CANVAS_API_URL}/accounts/1/sis_imports/${sisImport.id}`

  if (sisImport.workflow_state !== 'imported') {
    const errors = getSisErrors(sisImport.id)
